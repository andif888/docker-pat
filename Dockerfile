...
ENV TF_PROVIDER_LOCAL_VERSION=2.5.3
ENV TF_PROVIDER_NULL_VERSION=3.2.4
ENV TF_PROVIDER_TLS_VERSION=4.1.0
ENV TF_PROVIDER_VAULT_VERSION=5.1.0
ENV TF_PROVIDER_VSPHERE_VERSION=2.12.0
ENV TF_PROVIDER_CITRIX_VERSION=1.0.22
...
RUN curl -O https://releases.hashicorp.com/terraform/${TF_VERSION}/terraform_${TF_VERSION}_linux_amd64.zip \
    ...
    && mkdir -p /usr/share/terraform/plugins/registry.terraform.io/hashicorp/citrix \
    && curl -O --output-dir /usr/share/terraform/plugins/registry.terraform.io/hashicorp/citrix -L https://github.com/citrix/terraform-provider-citrix/releases/download/v${TF_PROVIDER_CITRIX_VERSION}/terraform-provider-citrix_${TF_PROVIDER_CITRIX_VERSION}_linux_amd64.zip \
